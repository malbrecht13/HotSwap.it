...
const addItemForTrade = async (req,res) => {
  ...
  try {
    ...
    if(!tradeItem) {
      return res.status(400).send({message: 'Trade item could not be saved'});
    }
    ...
    if(!userStore) {
      return res.status(400).send({message: 'The trade item could not be added'});
    }
    return res.status(201).send({itemAdded: tradeItem, userStore: userStore});

  } catch (e) {
    return res.status(500).send({message: 'Error while attempting to add item for trade'});
  }
}
